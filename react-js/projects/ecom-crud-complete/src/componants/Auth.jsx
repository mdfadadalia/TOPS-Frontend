...
export const Auth = () => {
  ...
  const submitLogin = async (event) => {
    ...
    if (loginUser.fulfilled.match(result)) navigate(location.state?.from?.pathname || '/')
  }
  const submitRegister = async (event) => {
    event.preventDefault()
    if (registerForm.password !== registerForm.confirmPassword) return
    const details = { name: registerForm.name, email: registerForm.email, password: registerForm.password }
